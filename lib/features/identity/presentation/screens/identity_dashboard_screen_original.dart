...
class _IdentityDashboardScreenState extends State<IdentityDashboardScreen> {
  ...
  Future<void> _authenticateUser() async {
    ...
    try {
      ...
      if (mounted) {
        _showErrorSnackbar('Authentication error: ${e.toString()}', context: context);
      }
    } finally {
      if (mounted) {
        setState(() {
        ...
        child: Column(
          ...
          children: [
            ...
            FadeInDown(
              ...
              child: _buildBlockchainDIDCard(context: context),
            ),
            const SizedBox(height: 24),
            FadeInDown(
            ...
            FadeInDown(
              ...
              child: _buildCredentialsList(credentials, context: context),
            ),
          ],
        ),
      ),
  ...
  Widget _buildCredentialsList(List<VerifiableCredential> credentials, {required BuildContext context}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(
        ...
        const SizedBox(height: 16),
        credentials.isEmpty
            ? _buildEmptyCredentialsState(context: context)
            : _buildCredentialsGrid(credentials, context: context),
      ],
    );
  }
  
  Widget _buildEmptyCredentialsState({required BuildContext context}) {
    return Center(
      child: Column(
        children: [
          const SizedBox(height: 24),
  ...
  Widget _buildCredentialsGrid(List<VerifiableCredential> credentials, {required BuildContext context}) {
    ...
    return GridView.count(
      ...
      children: previewCredentials.map((credential) {
          return _buildCredentialCard(credential, context: context);
        }).toList(),
    );
  }
  
  Widget _buildCredentialCard(VerifiableCredential credential, {required BuildContext context}) {
  ...
  Widget _buildBlockchainDIDCard({required BuildContext context}) {
  ...
  Widget _buildShimmerLoading() {
    return RefreshIndicator(
      onRefresh: () async {
        ...
        // Loading state is handled by the parent widget now
  ...
  void _showErrorSnackbar(String message, {bool isNetworkError = false, required BuildContext context}) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Row(
          children: [
        ...
        action: isNetworkError ? SnackBarAction(
          ...
          onPressed: () {
            // Handle retry logic here - would normally call _authenticateUser
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
